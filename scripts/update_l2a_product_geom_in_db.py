...
class Product(object) :
    def __init__(self, product_type_id, processor_id, satellite_id, site_id, orbit_id, name):
...
def update_product(product_dir):
    l2a_processed_tiles = []
    wkt = []
    sat_id = 0
    acquisition_date = ""
...
class L2AInfo(object):
    ...
    def get_l2a_prds(self, site_id):
        if not self.database_connect():
            return ""
        try:
            self.cursor.execute("select full_path from product where site_id = {}".format(site_id))
            rows = self.cursor.fetchall()
        except:
            print("Unable to get full_path from products for site_id = {}".format(site_id))
            self.database_disconnect()
            return ""
        self.database_disconnect()
        count = (len(rows))
        if count == 0 :
            print("L2A products for site_id = {} do not exist in product table".format(site_id))
            return None
...
prds_paths = l2a_db.get_l2a_prds_paths(site_id);

for prd_path in prds_paths:
    if os.path.isdir(prd_path):
        print("Updating product with full path: {}".format(str(prd_path)))
        update_product(prd_path)
    else :
        print("Ignoring product with full path: {}. It is not a directory!".format(str(prd_path)))
