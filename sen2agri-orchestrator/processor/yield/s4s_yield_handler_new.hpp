...
class S4SYieldHandlerNew : public ProcessorHandler
{
    typedef struct S4SYieldJobConfig {
        S4SYieldJobConfig(EventProcessingContext *pContext, const JobSubmittedEvent &evt)
            ...
            const std::map<QString, QString> &yfCfgParams = pCtx->GetJobConfigurationParameters(evt.jobId, S4S_YIELD_FEAT_CFG_PREFIX);
            suPath = ProcessorHandlerHelper::GetStringConfigValue(parameters, yfCfgParams, "su_path", S4S_YIELD_FEAT_CFG_PREFIX);
