...
void TimeSeriesReader::Build(std::vector<std::string>::const_iterator begin,
                             ...
                             const TileData &td)
{
    ...
    for (auto it = begin; it != end; ++it) {
        ...
        const std::unique_ptr<MetadataHelper<float, uint8_t>> &pHelper =
            m_ProductReaders[m_ProductReaders.size() - 1];
        ImageDescriptor id;
...
void TimeSeriesReader::updateRequiredImageSize(const std::vector<std::string> &descriptors,
                                               int startIndex,
                                               int endIndex,
                                               TileData &td)
{
    ...
    for (int i = startIndex; i < endIndex; i++) {
        ...
        std::unique_ptr<MetadataHelper<float, uint8_t>> pHelper =
            factory->GetMetadataHelper<float, uint8_t>(desc);
        if (pHelper->GetMissionShortName() == m_mission) {
            const std::vector<std::string> &firsResBandNames =
                pHelper->GetBandNamesForResolution(pHelper->GetProductResolutions()[0]);
            MetadataHelper<float, uint8_t>::VectorImageType::Pointer img =
                pHelper->GetImage(firsResBandNames);
...
void TimeSeriesReader::SetMission(std::string mission) { m_mission = std::move(mission); }

const std::string &TimeSeriesReader::GetMission() const { return m_mission; }

void TimeSeriesReader::SetPixelSize(float pixelSize) { m_pixSize = pixelSize; }

float TimeSeriesReader::GetPixelSize() const { return m_pixSize; }
...
void TimeSeriesReader::ProcessMetadata(
    const std::unique_ptr<MetadataHelper<float, uint8_t>> &pHelper,
...
void TimeSeriesReader::GetProductBands(
    const std::unique_ptr<MetadataHelper<float, uint8_t>> &pHelper,
    const TileData &td,
    ImageDescriptor &descriptor)
{
    ...
    MetadataHelper<float>::VectorImageType::Pointer img1 =
        pHelper->GetImage(bandNames1, &relBandIdxs1);
    MetadataHelper<float>::VectorImageType::Pointer img2 =
        pHelper->GetImage(bandNames2, &relBandIdxs2);
...
TimeSeriesReader::GetProductMask(const std::unique_ptr<MetadataHelper<float, uint8_t>> &pHelper,
                                 const TileData &td)
{
