...
//        paramOut->Write();
    ...
    AnglesImageType::Pointer allocateRaster()
    {
        ...
        anglesRaster->SetDirection(m_img->GetDirection());
        ...
        auto &metadata = anglesRaster->GetImageMetadata();
        std::vector<bool> flgs(3, true);
        std::vector<double> vals(3, NO_DATA_VALUE);
        otb::WriteNoDataFlags(flgs, vals, metadata);
