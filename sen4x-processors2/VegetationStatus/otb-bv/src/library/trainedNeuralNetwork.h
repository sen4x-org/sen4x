...
inline void read_one_line(std::ifstream &file, std::stringstream &ss) {
  std::string line;
...
inline double __purelin(double &x) {
  return x;
}
inline double __tansig(double &x) {
  return 2.0 / (1.0 + exp(-2.0*x) ) - 1.0;
}

inline function_double_to_double getFunc(std::string name) {
...
inline trained_network bvnet_fill_trained_network(std::string filename) {
